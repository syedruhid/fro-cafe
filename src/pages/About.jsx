...
import { Award, Users, Heart, MapPin, Clock, Phone, Mail } from 'lucide-react'
...
const About = () => {
  ...
  const EcoHome = () => {
    ...
    return (
      <>
        <div
          className="flex min-h-screen flex-row bg-[var(--warm-beige)] items-center justify-center "
        ...
        >
          <div className="max-w-[1200px] w-full text-center mb-12 ">
            ...
            <p className="text-[#6e8165] text-2xl max-w-3xl mx-auto leading-relaxed whitespace-pre-line">
          ...
          <div className="grid grid-cols-2 gap-12 max-w-[1200px] w-full px-4">
            {valuesData.map(({ title, description, imageUrl }) => (
              <ValueBox
                ...
                imageUrl={imageUrl}
                onClick={() => setSelectedCard({ title, description, imageUrl })}
              />
            ))}
          </div>
        </div>
        ...
        {selectedCard && (
          <Modal onClose={() => setSelectedCard(null)}>
            <div className="max-w-xl w-full bg-white rounded-lg shadow-lg p-8 relative">
              ...
              />
              <h2 className="text-3xl font-bold text-[#141712] mb-4">{selectedCard.title}</h2>
              <p className="text-[#6e8165] text-lg leading-relaxed">{selectedCard.description}</p>
            </div>
          </Modal>
        )}
      </>
