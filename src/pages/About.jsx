...
const About = () => {
  ...
  const EcoHome = () => {
    ...
    return (
      <>
        <div
          className="flex min-h-screen flex-col bg-[var(--warm-beige)] items-center justify-center pt-36 pb-20"
          style={{ fontFamily: 'Inter, "Noto Sans", sans-serif' }}
        >
          <div className="w-full text-center mb-12 pt-10">
            <h1 className="text-[#141712] text-5xl font-bold mb-6">
              About us–<span className="bg-[var(--walnut-brown)] text-white px-2 py-1 rounded">FRØ Café </span>
            </h1>
            <p className="text-[#6e8165] text-xl max-w-3xl pt-10 mx-auto leading-relaxed whitespace-pre-line">
              {
          ...
          <div className="grid grid-cols-2 gap-12 max-w-[1200px] w-full px-4">
            {valuesData.map(({ title, description, imageUrl }) => (
              <ValueBox
                ...
                imageUrl={imageUrl}
                onClick={() =>
                  setSelectedCard({ title, description, imageUrl })
                }
              />
            ))}
          </div>
        ...
        {selectedCard && (
          <Modal onClose={() => setSelectedCard(null)}>
            <div className="max-w-xl w-full bg-white rounded-lg shadow-lg p-8 relative">
              ...
              />
              <h2 className="text-3xl font-bold text-[#141712] mb-4">
                {selectedCard.title}
              </h2>
              <p className="text-[#6e8165] text-lg leading-relaxed">
  ...
  function ValueBox({ title, description, imageUrl, onClick }) {
    return (
      <div
        tabIndex={0}
      ...
      >
        ...
        <p className="text-[#6e8165] text-lg leading-relaxed truncate">
          {description}
        </p>
      </div>
    );
